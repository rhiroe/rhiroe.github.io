...
チームではこれまでスクラム開発が採用されていましたが、スクラムマスターの不在やプロダクトオーナーが複数チームを掛け持ちしている現状を考慮し、スクラムをやめカンバンをメインにした開発者主体での開発スタイルに変更しました。課題としてはイベントに開発者しかいないためプロダクト視点の欠落やゴールがタスクベースで決まる(例:〇〇を終わらせる)などがあり、これをカンバンによって解決する狙いがありました。
カンバンのチケットをユーザーストーリー単位に強制することで、バックログをユーザーストーリーで分割することを強制させます。その過程で意識がタスクよりも顧客への価値へと向くため自然とプロダクト視点を持てるようになります。スクラムを明確にやめることでスクラムマスターやプロダクトオーナーという概念をチームから抹消し、特定の役割にとらわれない臨機応変な活動を促します。
スクラムとカンバンを融合させることも考えましたが、スクラムマスターやプロダクトオーナーがイベントに参加できないのであればスクラムはやめるべきだと判断しました。でもやってること自体は実はそんなに変わらない。

#### 請求関連機能のバージョン管理

請求書や利用明細、またそれに付随するCSVなどの請求関連データは、消費税やインボイスなど行政によって計算方法やフォーマットの変更を余儀なくされることがあります。システムの変更はその度に発生しますが、過去の情報は当時の計算ロジックやフォーマットで出力しなければならないという制約もあります。
これまでの請求機能は一つの機能の中で日付による条件分岐でこれらの計算ロジックやフォーマットが選択され、ロジックを組み合わせていくような複雑な実装がされていましたが、これを計算ロジックやフォーマットが変わるごとにバージョンを分け、異なるネームスペースで完全に分離させるよう再設計しました。
一つのバージョンへの変更の影響が他のバージョンへ波及しないことを優先し、異なるバージョン間で同じロジックがあったとしても、あえて共通化しないという設計を行っています。これにより現在の機能への改修や新しいフォーマット・計算ロジックでの実装を過去のバージョンへの影響を考えることなく行えるようにしています。

#### 休業手当対応

厚労省からの注意喚起により企業側の急なマッチングキャンセルに対して休業手当を支給するような仕組みを構築しました。休業手当を正しく支給するためにキャンセル理由を細かくカテゴライズし、選ばれた理由によって休業手当の有無を正確に判定しています。また、休業手当の発生しないキャンセルであったとしてもワーカーから異議申し立てが可能な動線と、労使間で協議が行える機能を新たに開発しました。この労使間協議の機能は休業手当に留まらず、今後のあらゆる労使間での話し合いに使用できる拡張性を備えてあります。
...
SteepでStrictに加えカスタムでいくつか検査項目を追加したものを設定し、厳密な型検査を行なっています。実装の90%以上に正確な型コメントが書かれています。この恩恵もあり、本番環境ではリリース以降一度も実装ミスによる500番台のエラーは発生していません。
RubyやRailsの開発バージョンでのCIをデイリーで回しています。これにより次回バージョンでエラーになる箇所を先回りして修正したり、RubyやRailsのコミュニティーにバグレポートをフィードバックしたりできています。

#### DevContainerでの開発環境整備

DevContainerでの開発環境を整備しました。型システム周りの準備であったり、環境変数の準備やステージング環境へのアクセスに必要なソフトウェアのインストールなど、`devcontainer open`のワンコマンドで全てが完了します。
...
#### 企業・店舗情報管理ツールの移管 HubSpot -> Salesforce

クライアント情報の管理ツールをHubSpotからSalesforceへ移管する作業を行いました。冪等性を持たせリトライが可能なよう実装し、外部システムとのやりとりをエラーなく行えるよう再設計・リファクタリング・実装しました。
